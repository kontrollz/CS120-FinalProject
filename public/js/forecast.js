...
let prefs = [new Pref("timepoint", false), new Pref("cloudcover", false), new Pref("seeing", false), new Pref("transparency", true), new Pref("lifted_index", true), new Pref("rh2m", true), new Pref("wind10m", true), new Pref("temp2m", true), new Pref("prec_type", true)];
...
async function get_data() {
    // fetch api in backend instead
    let response = await fetch(`/forecast/api?lat=${lat}&lon=${lon}`);
    ...
    timet = (String(forecastData.init).slice(0, 4) + "-" + String(forecastData.init).slice(4, 6) + "-" + String(forecastData.init).slice(6, 8) + " " + String(forecastData.init).slice(8,10) + ":00");
    // document.getElementById('dateloc').innerHTML += ("<h2 id='subtitle'>Projection Time: " + timet + ", Location: (" + lon + ", " + lat + ")");
    document.getElementById('proj-time').textContent = timet;
    document.getElementById('proj-loc').textContent = "(" + lon + ", " + lat + ")";
    ...
    // alert(JSON.stringify(forecastData.dataseries[0]));
    forecastData.dataseries.forEach ( (x) => {
    ...
    modules_arr[0].label.forEach ( (x, index) => {
        var1 = ((x) + Number(forecastData.init));
...
modules_arr[6].label.forEach ( (x, index) => {
    ...
    modules_arr[0].label.forEach( (x, index) => {
        selector.innerHTML += ("<option>" + x + " </option>");
    });
    ...
    modules_arr.forEach ( (x) => {
        if (x.show == true) {
        document.getElementById('boxes').innerHTML += ("<div class='module-box'><h1>" + x.title + "</h1><br /><h2>" + x.label[i] + "</h2></div>");
        }
        })
    });
